...
by the *inverse* of its local environmental density \\w(\mathbf{x}) =
1/\hat{g}(\mathbf{x})\\. This converts the discrete sum over the
heterogeneous background grid into a numerical approximation of a
Lebesgue-measure integral, effectively projecting the niche onto a
uniform environmental space.
...
drawn from the fundamental niche density \\f(\mathbf{x};\mu,\Sigma)\\
*restricted* to the set of environments that exist in \\M\\:

\\ p(\mathbf{x}\_i \mid \text{presence in } M) =
\frac{f(\mathbf{x}\_i;\mu,\Sigma)} {\int_M
f(\mathbf{x};\mu,\Sigma)\\d\mathbf{x}}. \tag{1} \\
...
### 2.3 Addressing the DeepSeek Critique

A recent analysis (anonymous reviewer using DeepSeek, May 2026) argued
that the IPW denominator is statistically flawed because it does not
correspond to the use-availability DGP (\\\lambda \propto f \cdot g\\).
This critique is **internally consistent under its assumed DGP** but
does not engage with the Jimenez DGP (Eq. 1), under which the IPW
denominator is a valid Horvitz–Thompson estimator of \\\int_M
f\\d\mathbf{x}\\.

Specifically:

1.  **The denominator** \\\sum_j
    f(\mathbf{y}\_j)/\hat{g}(\mathbf{y}\_j)\\ approximates \\\int_M
    f(\mathbf{x})\\d\mathbf{x}\\ by importance sampling, where the
    background points \\\\\mathbf{y}\_j\\\\ are treated as a sample from
    \\g\\.

2.  **The sign convention** (\\-\log\hat{g}\\ in the denominator,
    \\+\log\hat{g}\\ in the numerator) is the algebraic consequence of
    \\w = 1/\hat{g}\\.

3.  **Self-regularisation** against \\\sigma \to \infty\\ arises because
    the Horvitz–Thompson sum amplifies low-density tails where
    \\1/\hat{g}\\ is large, making the denominator grow faster than the
    numerator.
...
Starting from Equation 8 of Jimenez and Soberón (2022) with \\w =
1/\hat{g}\\:

\\ \ell(\mu,\Sigma) = \sum\_{i=1}^{n}\log f(\mathbf{x}\_i) -
\sum\_{i=1}^{n}\log\hat{g}(\mathbf{x}\_i) -
n\\\log\\\sum\_{j=1}^{K}\frac{f(\mathbf{y}\_j)}{\hat{g}(\mathbf{y}\_j)}.
\tag{2} \\
...
\sum\_{i=1}^{n}\log\hat{g}(\mathbf{x}\_i) +
n\cdot\operatorname{logsumexp}\_j\\\left\[ -\tfrac{1}{2}q_j -
\log\hat{g}(\mathbf{y}\_j) \right\], \tag{3} \\
...
The denominator \\\sum_j \exp(-\frac{1}{2}q_j -
\log\hat{g}(\mathbf{y}\_j))\\ is equivalent to the Horvitz–Thompson
estimator:

\\ \widehat{\int_M f\\d\mathbf{x}} =
\frac{1}{K}\sum\_{j=1}^{K}\frac{f(\mathbf{y}\_j)}{\hat{g}(\mathbf{y}\_j)},
\tag{4} \\

which converges to \\\int_M f(\mathbf{x})\\d\mathbf{x}\\ as \\K \to
\infty\\ whenever \\\hat{g}(\mathbf{y}\_j) \> 0\\ for all \\j\\ in the
support of \\f\\ (Horvitz and Thompson (1952); Hahn (1998)).
...
4.  Computes the integrand \\a_j = -0.5 q_j -
    \log\hat{g}(\mathbf{y}\_j)\\ (line 143).
...
- **\\\nabla\_\mu\\**: Closed-form via \\\Sigma^{-1}\[\text{weighted
  mean shift}\]\\, with softmax weights \\\pi_j =
  \exp(a_j)/\sum_k\exp(a_k)\\ on the background side (lines 263–271).
- **\\\nabla\_{\log\sigma_k}\\**: Closed-form via \\U \odot V\\ products
  (lines 273–278).
- **\\\nabla\_{v_k}\\**: Central finite differences on the C-vine block
  (lines 280–298).
...
1.  As \\\sigma \to \infty\\, \\f(\mathbf{y}\_j) \to \text{const}\\ for
    all \\j\\.
2.  The denominator becomes \\\sum_j
    \text{const}/\hat{g}(\mathbf{y}\_j)\\, which is large because
    \\1/\hat{g}\\ amplifies low-density tail points.
...
4.  The net effect is that \\-\ell^\*\\ *increases* as \\\sigma \to
    \infty\\, providing a natural penalty.
...
1.  **Variance amplification**. When \\\hat{g}(\mathbf{y}\_j) \approx
    0\\ (environmental deserts), the weight \\1/\hat{g}\\ can be
    extremely large, inflating the variance of the estimator.
    Diagnostic: check the effective sample size \\\text{ESS} = (\sum_j
    w_j)^2 / \sum_j w_j^2\\.
