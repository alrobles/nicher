...
SEXP create_niche_obj_ptr(
  ...
  else if (likelihood == "ip_weighted")           lt = LIK_KDE_BIAS_CORRECTED;
  ...
  if (grad == "analytic") {
    if (lt != LIK_KDE_BIAS_CORRECTED && lt != LIK_WEIGHTED) {
      ...
      // weighted family; warn and downgrade to central FD for the others
      // (skew-normal, skew-t, presence_only, unweighted).
      Rcpp::warning(
        "grad = 'analytic' is not yet implemented for likelihood = '%s'; "
    ...
    [state](const std::vector<double>& x,
      ...
      try {
        if (state->grad_mode == GRAD_ANALYTIC && state->lik_type == LIK_KDE_BIAS_CORRECTED) {
          f = nicher::loglik_niche_math_ip_weighted_grad_eigen(
                x.data(), n,
