...
.cv_make_neg_loglik <- function(likelihood, env_occ_sub, env_m, args,
  ...
  switch(
    ...
    stop("Unknown likelihood in .cv_make_neg_loglik: ", likelihood)
  )
}
...
.cv_refit_fold <- function(theta_init, train_occ, env_m, likelihood, args,
  ...
  weighted_fams <- c("ip_weighted", "weighted",
...
cv_nicher <- function(fit, env_occ, env_m = NULL,
  ...
  n_failed <- sum(is.na(per_fold$loglik_test))
  if (n_failed > 0L) {
    warning(n_failed, " fold(s) returned NA and were excluded from the cross-validation log-likelihood.")
  }
  cv_total <- sum(per_fold$loglik_test, na.rm = TRUE)
  out <- list(
    cv_loglik       = cv_total,
