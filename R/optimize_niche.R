...
#'
#' @seealso [loglik_niche_math_cpp()], [loglik_niche_math_presence_only()],
...
optimize_niche <- function(env_occ,
                           ...
                           verbose = FALSE,
                           ...) {
  # Accept the deprecated alias silently
  if (!missing(likelihood) && is.character(likelihood) &&
      length(likelihood) == 1L && likelihood == "kde_bias_corrected") {
  ...
  # family (ip_weighted, weighted) only; the skew-normal and skew-t
  ...
  ws <- .run_warm_start(
    warm_start = warm_start, is_weighted_family = is_weighted_family,
    is_skew_family = is_skew_family, is_skew_t_family = is_skew_t_family,
    env_occ = env_occ, num_starts = num_starts, breadth = breadth,
    grad = grad, seed = seed, control = control, verbose = verbose,
    p_occ = p_occ, starts_list = starts_list, ...
  )
  starts_list <- ws$starts_list

  priors <- .resolve_prior_centers(
    prior_log_sigma_center = prior_log_sigma_center,
    prior_mu_center = prior_mu_center,
    po_anchor_log_sigma = ws$po_anchor_log_sigma,
    po_anchor_mu = ws$po_anchor_mu,
    is_weighted_family = is_weighted_family,
    prior_log_sigma_lambda = prior_log_sigma_lambda,
    prior_mu_lambda = prior_mu_lambda,
    env_occ = env_occ, p_occ = p_occ
  )
  prior_log_sigma_center <- priors$prior_log_sigma_center
  prior_mu_center        <- priors$prior_mu_center
  ...
  compiled <- .compile_solutions(results)
  ...
  best <- .build_ic_payload(
    best = best, likelihood = likelihood, env_occ = env_occ,
    env_m = env_m, weighted_inputs = weighted_inputs, eta = eta
  )
...
# ===========================================================================
# Internal: warm-start presence-only fit and pad theta for skew families
# ===========================================================================

#' @keywords internal
.run_warm_start <- function(warm_start, is_weighted_family,
                            is_skew_family, is_skew_t_family,
                            env_occ, num_starts, breadth,
                            grad, seed, control, verbose,
                            p_occ, starts_list, ...) {
  po_anchor_mu        <- NULL
  po_anchor_log_sigma <- NULL

  if (!(warm_start && is_weighted_family)) {
    return(list(starts_list = starts_list,
                po_anchor_mu = po_anchor_mu,
                po_anchor_log_sigma = po_anchor_log_sigma))
  }
...
# ===========================================================================
# Internal: resolve default centres for ridge priors
# ===========================================================================

#' @keywords internal
.resolve_prior_centers <- function(prior_log_sigma_center, prior_mu_center,
                                   po_anchor_log_sigma, po_anchor_mu,
                                   is_weighted_family,
                                   prior_log_sigma_lambda, prior_mu_lambda,
                                   env_occ, p_occ) {
  ...
  list(prior_log_sigma_center = prior_log_sigma_center,
       prior_mu_center = prior_mu_center)
}


# ===========================================================================
# Internal: compile multi-start results and select the best solution
# ===========================================================================

#' @keywords internal
.compile_solutions <- function(results) {
...
# ===========================================================================
# Internal: attach IC payload (unpenalised loglik, nobs, fingerprints)
# ===========================================================================

#' @keywords internal
.build_ic_payload <- function(best, likelihood, env_occ, env_m,
                              weighted_inputs, eta) {
