...
#' @param likelihood Character. One of \code{"ip_weighted"} or
#'   \code{"presence_only"}.
...
assess.nicher <- function(x,
                          ...
                          ...) {
  if (min_converged < 1L) {
    stop("`min_converged` must be at least 1.")
  }
  sols   <- x$solutions
  ...
  if (is.na(rel_gap)) {
    flag <- "accepted_noise"
    rec  <- paste0(
      "Single converged solution accepted; gap analysis requires at least ",
      "two converged starts and cannot be performed."
    )
  } else if (rel_gap <= tol_gap) {
