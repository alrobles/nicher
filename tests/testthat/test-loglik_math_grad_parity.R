test_that("hybrid analytic gradient matches central FD", {
  ...
  fd_grad <- function(theta) {
    ...
    for (i in seq_len(n)) {
      ...
      th_m <- theta
      th_m[i] <- theta[i] - dx
      fp <- nicher:::loglik_niche_math_ip_weighted_cpp(
        th_p, occ, M_den, w_occ, w_den, eta = 1)
      fm <- nicher:::loglik_niche_math_ip_weighted_cpp(
  ...
  for (k in seq_len(5L)) {
    theta <- start_theta(occ) + stats::rnorm(length(start_theta(occ)),
                                             sd = 0.05)
    res <- nicher:::loglik_niche_math_ip_weighted_grad_cpp(
...
test_that("penalized weighted hybrid analytic gradient matches central FD", {
  occ <- as.matrix(example_env_occ_2d)
  M   <- as.matrix(example_env_m_2d)
