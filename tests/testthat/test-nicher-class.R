...
.make_mock_nicher <- function(logliks, convs, likelihood = "ip_weighted") {
  n <- length(logliks)
...
test_that("weighted warm start reports actual number of starts", {
  fit <- optimize_niche(
  ...
  expect_equal(fit$n_starts, nrow(fit$solutions))
  expect_equal(fit$fit_args$num_starts, 2L)
  expect_match(capture.output(print(fit))[3L], "Starts\\s+: 3")
})
