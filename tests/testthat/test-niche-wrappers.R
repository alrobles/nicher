...
#   niche_presence_only(), niche_ip_weighted()
#
...
# Precomputed KDE weights for niche_ip_weighted tests
set.seed(123)
...
# ---------------------------------------------------------------------------
# Test 5 – niche_ip_weighted returns correct structure
# ---------------------------------------------------------------------------
test_that("niche_ip_weighted returns a list with value, conv, and theta", {
  res <- niche_ip_weighted(
    occ           = occ3,
    M             = M3,
    den_idx       = den_idx3,
    kde_idx       = kde_idx3,
...
# ---------------------------------------------------------------------------
# Test 6 – niche_ip_weighted returns a finite log-likelihood
# ---------------------------------------------------------------------------
test_that("niche_ip_weighted returns a finite log-likelihood", {
  res <- niche_ip_weighted(
    occ           = occ3,
    M             = M3,
    den_idx       = den_idx3,
  ...
  expect_true(is.finite(res$value),
    label = "niche_ip_weighted value is finite"
  )
})
...
# ---------------------------------------------------------------------------
test_that("niche_ip_weighted errors when precomp_w_den length does not match den_idx", {
  expect_error(
    niche_ip_weighted(
      occ           = occ3,
      M             = M3,
...
test_that("wrappers error on non-finite starting values", {
  ...
  expect_error(
    niche_ip_weighted(occ3, M3, den_idx3, kde_idx3, pre3, start = bad_start),
    regexp = "finite",
    label  = "niche_ip_weighted errors on Inf start"
  )
})
