...
test_that("loglik_niche_math_weighted_penalized_cpp respects ridge prior", {
  occ <- as.matrix(example_env_occ_2d)
  M   <- as.matrix(example_env_m_2d)
  p   <- ncol(occ)
  ...
  for (k in seq_len(15L)) {
    ...
    f_lambda0 <- nicher:::loglik_niche_math_weighted_penalized_cpp(
    ...
    f_lambda7 <- nicher:::loglik_niche_math_weighted_penalized_cpp(
      theta, occ, M_den, w_occ, w_den,
    ...
    f_at_centre_l0 <- nicher:::loglik_niche_math_weighted_penalized_cpp(
      theta_at_centre, occ, M_den, w_occ, w_den,
      prior_log_sigma_center = log_sd, prior_log_sigma_lambda = 0.0,
      eta = 1.0
    )
    f_at_centre_l5 <- nicher:::loglik_niche_math_weighted_penalized_cpp(
      theta_at_centre, occ, M_den, w_occ, w_den,
